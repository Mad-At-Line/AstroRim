...
def generate_realistic_simulation(image_size=64):
    ...
    for lens_type in lens_types:
        if lens_type == 'SIE':
            ...
            kwargs_lens.append({
                'gamma1': np.random.uniform(-0.3, 0.3),
                'gamma2': np.random.uniform(-0.3, 0.3)
            })
...
if __name__ == '__main__':
    output_dir = r'C:\Users\mythi\.astropy\Code\Fits_work\fits_4.75_data1'
    ...
    for i in range(num_simulations):
        ...
        # Make FITS with both arrays
        hdu_primary = fits.PrimaryHDU()  # empty primary
        hdu_gt = fits.ImageHDU(gt, name='GT')
        hdu_lensed = fits.ImageHDU(lensed, name='LENSED')

        hdul = fits.HDUList([hdu_primary, hdu_gt, hdu_lensed])

        filename = os.path.join(output_dir, f'simulation4.5_{i:04d}.fits')
        hdul.writeto(filename, overwrite=True)

        print(f"Simulation {i + 1} saved to {filename}")
