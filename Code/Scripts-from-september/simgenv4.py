#!/usr/bin/env python3
"""
Modified realistic strong-lensing simulator (fastell4py-free)

- Avoids PEMD/SPEMD so fastell4py is NOT required.
- Uses SIE (and SIS/NFW/SHEAR) for lens mass models.
- Reduced maximum noise and expanded lens diversity as requested.
- Keeps two HDUs: Primary (GT unlensed, detector-sampled) and ImageHDU (LENSED: lensed + lens light + PSF + noise + artifacts)
"""
...
OUTPUT_DIR = r"C:\Users\mythi\.astropy\Code\Fits_work\dataset_mixed_120k"
...
USE_PEMD_PROB = 0.0        # <-- set to zero to avoid PEMD entirely (no fastell4py needed)
...
def generate_one(i, outdir=OUTPUT_DIR):
    ...
    # Use SIE for the main lens to avoid PEMD/SPEMD and fastell4py requirement
    ...
    filename = os.path.join(outdir, f"realistic_rim4_{i:05d}.fits")
