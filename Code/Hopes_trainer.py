#WORKING DO NOT TOUCH DO NOT FUCKING TOUCH ME YOU SICKO, COPY ME BEFORE YOU EDIT GRRRRR HANDS OFF
...
class RIM(nn.Module):
    def __init__(self, n_iter=15, hidden_dim=64):
        super().__init__()
        self.n_iter = n_iter
        self.hidden_dim = hidden_dim
...
def train_rim():
    ...
    dataset_dir = r"C:\Users\mythi\.astropy\Code\Hopes_and_dreams\data_15k"
    ...
    model = RIM(n_iter=10, hidden_dim=64).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        print(f"[Epoch {epoch+1}] Train Loss: {avg_train_loss:.6f} | Val Loss: {avg_val_loss:.6f} | Val SSIM: {avg_val_ssim:.4f} | Grad Norm: {avg_grad_norm:.2f}")
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("loss_and_grad_curve.png", dpi=300)
    plt.show()
