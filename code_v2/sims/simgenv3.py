"""
simgenv3.py - AstroRIM v2 (patched).

Fixes vs original:
  * Moffat alpha formula corrected. Previous code produced Moffat kernels ~15% narrower
    than the stated FWHM whenever PSF_TYPES sampled 'MOFFAT' (which is ~50% of samples).
  * Lens-light ellipticity now aligned with mass ellipticity (was orthogonal).

Sample filename prefix: rim_simv3_{i:05d}.fits
...
def generate_one(i, outdir=OUTPUT_DIR):
    ...
    filename = os.path.join(outdir, f"rim_sim3_{i:05d}.fits")
    ...
    sgt.write_truth_header(phdr, truth, maskbits, clean, sim_set='simgenv3',
