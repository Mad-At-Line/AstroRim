...
INSTRUMENTS = [
    (0.45, (0.06, 0.12), (0.006, 0.20), (0.5, 1.5), 0.04),  # space-like
    (0.40, (0.5, 1.2), (0.02, 0.8), (0.8, 2.5), 0.20),     # ground-like
    (0.15, (0.12, 0.35), (0.01, 0.3), (0.6, 1.2), 0.08),   # intermediate
]

SIGMA_KMS_RANGE = (80.0, 380.0)
ELLIPTICITY_RANGE = (0.0, 0.7)
SHEAR_MAX = 0.16

SRC_MAG_RANGE = (20.0, 25.0)
LENS_MAG_DELTA_RANGE = (-2.0, +3.0)

PSF_TYPES = ['GAUSSIAN', 'MOFFAT']
PSF_MOFFAT_BETA_RANGE = (2.8, 5.0)
PSF_ELLIP_MAX = 0.35

GAIN = 1.0
READ_NOISE_RANGE = (0.5, 3.0)
SKY_ADU_RANGE = (0.005, 1.0)
PRNU_RMS = 0.004
SKY_GRADIENT_MAX = 0.02
COSMIC_RAY_PROB = 0.01
COSMIC_RAY_INTENSITY = (15.0, 200.0)

ADD_GROUP_HALO_PROB = 0.22
ADD_SUBHALOS_PROB = 0.75

ADD_CORRELATED_NOISE_PROB = 0.5
CORRELATED_NOISE_SIGMA_RANGE = (0.6, 2.5)
ADD_SKY_SUBTRACTION_RESIDUALS_PROB = 0.35
ADD_SUBPIXEL_SHIFT_PROB = 0.75
ADD_PSF_MISMATCH_PROB = 0.5

N_EXTRA_SOURCES_RANGE = (0, 4)
USE_PEMD_PROB = 0.0        # <-- set to zero to avoid PEMD entirely
DOUBLE_MAIN_LENS_PROB = 0.08

SUPER_SIZE = IMAGE_SIZE * OVERSAMPLE
# SUP_PIXEL_SCALE is defined per-generation because pixel scale may change per instrument
# SUPER_PIX_AREA likewise.
...
def build_sources(x_sup, y_sup, theta_E):
    ...
    if np.random.rand() < 0.75:
        main = add_clumps(main, n=np.random.randint(1, 6), max_rel=np.random.uniform(0.12, 0.6),
...
def generate_one(i, outdir=OUTPUT_DIR):
    ...
    z_l = float(np.random.uniform(0.15, 0.9))
    z_s = float(np.random.uniform(max(z_l + 0.05, 0.6), 3.5))
    ...
    # Use SIE for the main lens to avoid PEMD/SPEMD
    lens_model_list.append('SIE')
    kwargs_lens.append({
        'theta_E': theta_E,
        'e1': e1, 'e2': e2,
        'center_x': np.random.uniform(-0.06, 0.06),
        'center_y': np.random.uniform(-0.06, 0.06)
    })

    # optionally add a second main lens (galaxy pair)
    if np.random.rand() < DOUBLE_MAIN_LENS_PROB:
        thetaE2 = theta_E * np.random.uniform(0.25, 0.9)
        e2_val = float(np.random.uniform(*ELLIPTICITY_RANGE))
        phi2 = np.random.uniform(0, np.pi)
        e12 = e2_val * np.cos(2 * phi2)
        e22 = e2_val * np.sin(2 * phi2)
        lens_model_list.append('SIE')
        kwargs_lens.append({
            'theta_E': thetaE2,
            'e1': e12, 'e2': e22,
            'center_x': kwargs_lens[0]['center_x'] + np.random.uniform(-0.6*theta_E, 0.6*theta_E),
            'center_y': kwargs_lens[0]['center_y'] + np.random.uniform(-0.6*theta_E, 0.6*theta_E)
        })

    # external shear optional
    if shear_g > 0.0:
    ...
    if np.random.rand() < ADD_GROUP_HALO_PROB:
        ...
        kwargs_lens.append({
            # lenstronomy NFW expects certain kwargs; this is a heuristic set
            'alpha_Rs': np.random.uniform(0.2, 1.2) * 0.8,
            'Rs': np.random.uniform(8.0, 40.0),
            'center_x': kwargs_lens[0]['center_x'] + np.random.uniform(-1.0*theta_E, 1.0*theta_E),
            'center_y': kwargs_lens[0]['center_y'] + np.random.uniform(-1.0*theta_E, 1.0*theta_E)
        })

    # optional subhalos (SIS)
    if np.random.rand() < ADD_SUBHALOS_PROB:
        n_sub = np.random.poisson(1.4)
        for _ in range(max(0, n_sub)):
            lens_model_list.append('SIS')
            kwargs_lens.append({
                'theta_E': np.random.uniform(0.006, 0.08),
                'center_x': np.random.uniform(-1.0 * theta_E, 1.0 * theta_E),
                'center_y': np.random.uniform(-1.0 * theta_E, 1.0 * theta_E)
            })

    # initialize LensModel (no PEMD used)
    ...
    ll_Re_arcsec = np.random.uniform(0.25, 1.2)
    ll_n_sersic = np.random.uniform(2.5, 5.0)
    ...
    true_psf_sup, psf_kernel_info = make_psf_kernel(psf_fwhm, SUP_PIXEL_SCALE, psf_type, ellip=psf_ellip, angle=psf_angle, beta=psf_beta)
    if np.random.rand() < ADD_PSF_MISMATCH_PROB:
        meas_fwhm = psf_fwhm * np.random.uniform(0.92, 1.20)
        meas_ellip = min(PSF_ELLIP_MAX, max(0.0, psf_ellip + np.random.normal(0.0, 0.05)))
        meas_angle = psf_angle + np.random.uniform(-0.35, 0.35)
        measured_psf_sup, _meas_psf_info = make_psf_kernel(meas_fwhm, SUP_PIXEL_SCALE, psf_type, ellip=meas_ellip, angle=meas_angle, beta=psf_beta)
    else:
    ...
    if np.random.rand() < 0.20 and ratio < 0.25:
        LENSED_counts *= 0
        lens_counts_sup *= np.random.uniform(0.85, 1.0)
        lensed_src_counts_sup *= np.random.uniform(1.05, 1.4)
        image_sup = lensed_src_counts_sup + lens_counts_sup
        image_conv_sup = convolve_fft(image_sup, true_psf_sup, normalize_kernel=True, allow_huge=True)
        if np.random.rand() < ADD_SUBPIXEL_SHIFT_PROB:
            dx = np.random.uniform(-0.4, 0.4)
    ...
    if np.random.rand() < ADD_CORRELATED_NOISE_PROB:
        ...
        corr *= np.random.uniform(0.001, 0.01) * max(1.0, np.median(LENSED_counts_prnu + 1e-12))
        LENSED_counts_prnu += corr

    if np.random.rand() < ADD_SKY_SUBTRACTION_RESIDUALS_PROB:
        ax = np.linspace(-1, 1, IMAGE_SIZE)
        bx = np.random.uniform(-0.0015, 0.0015)
        by = np.random.uniform(-0.0015, 0.0015)
    ...
    filename = os.path.join(outdir, f"rim_simv4_{i:05d}.fits")
    ...
    # [FIX] 'PSF_FWH_TRUE' exceeded the 8-char FITS keyword limit
    phdr['PSFW_TRU'] = (psf_fwhm, 'simulated true PSF FWHM (arcsec)')
    phdr['PSFW_MEA'] = (meas_fwhm, 'measured (mismatched) PSF FWHM (arcsec)')
    ...
    phdr['ELLIP'] = (e, 'main lens ellipticity')
    ...
    phdr['NOISECFG'] = ('reduced_max', 'noise configuration')
    ...
    ll_amp = sgt.measure_lens_light_amp_at_Re(
        ...
        PIXEL_SCALE_THIS, norm_m)
    truth, maskbits, clean = sgt.truth_from_lenstronomy(
        lens_model_list, kwargs_lens,
        pixel_scale=PIXEL_SCALE_THIS, image_size=IMAGE_SIZE,
        psf_info={
            'realized_fwhm_pix': realized_fwhm_arcsec / PIXEL_SCALE_THIS,
    ...
    sgt.write_truth_header(phdr, truth, maskbits, clean, sim_set='simgenv4',
                           pixel_scale=PIXEL_SCALE_THIS, coma_applied=False)
