...
IMAGE_SIZE = 96          # final detector pixels = 96 x 96
...
def generate_one(i, outdir=OUTPUT_DIR):
    ...
    ll_Re_arcsec = np.random.uniform(0.18, 0.9)
    ll_n_sersic = np.random.uniform(2.5, 5.0)
    ll_center_x = center_x + np.random.uniform(-0.02, 0.02)
    ll_center_y = center_y + np.random.uniform(-0.02, 0.02)
    ...
    # Save to FITS
    filename = os.path.join(outdir, f"rim_simv5_{i:05d}.fits")
    ...
    sgt.write_truth_header(hdr, truth, maskbits, clean, sim_set='simgenv5',
