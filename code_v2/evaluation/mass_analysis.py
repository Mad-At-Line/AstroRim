...
# FORWARD MODEL: imported from astrorim_core [FIX M1]
# (single source of truth shared with training.py / recon_gen.py; the
# previous ~650-line in-file copy is gone. compute_convergence_map is a
# method of the core ConditionalPhysicalForward.)
from astrorim_core import (
    TRUTH_ORDER, TRUTH_FITS_KEYS,
    ConditionalPhysicalForward,
    load_checkpoint_file, pack_truth_from_header,
    DEFAULT_PSF_FWHM_RANGE, LEGACY_PSF_FWHM_RANGE,
)
# END OF FORWARD MODEL IMPORT
...
def load_fits_pair(fits_path, source_hdu_names=None):
    ...
    lensed = None
    source = None
    # [FIX M2] capture header metadata: pixel-scale candidates + TR_* truth
    ...
    for i, h in enumerate(hdul):
        ...
        # Check for observed lensed image
            ...
            if lensed is None:  # Take first lensed image found
                lensed = d
                lensed_hdr = h.header.copy()  # [FIX M2]
    ...
    # [FIX M2] assemble header metadata ------------------------------------
    def _scale_candidates(hdrs):
        cands = []
        for hdr in hdrs:
            if hdr is None:
                continue
            for key in ('PP_PS_N', 'TR_PS', 'PIXSCALE', 'PIXSCAL1'):
                if key in hdr:
                    try:
                        v = float(hdr[key])
            ...
            for key in ('CD1_1', 'CDELT1'):
                if key in hdr:
                    try:
                        v = abs(float(hdr[key])) * 3600.0
                        if np.isfinite(v) and 1e-4 < v < 30.0:
                            cands.append((key + '*3600', v))
                    except Exception:
                        pass
    ...
    header_meta = {
        'pixscale_candidates': _scale_candidates([primary_hdr, lensed_hdr]),
        'pp_ps_orig': float(primary_hdr['PP_PS_O']) if 'PP_PS_O' in primary_hdr else None,
        'truth': None,
    }
    try:
        tvec, tmask, tinfo = pack_truth_from_header(primary_hdr)
        if tmask.sum() > 0:
            header_meta['truth'] = {
                'values': {k: float(tvec[i]) for i, k in enumerate(TRUTH_ORDER)},
                'mask': {k: int(tmask[i]) for i, k in enumerate(TRUTH_ORDER)},
                'clean': tinfo.get('clean', 0),
                'sim_set': tinfo.get('sim_set', ''),
            }
    except Exception:
        pass

    logger.info(f"Loaded FITS: lensed shape {lensed.shape}, source {'found' if src_tensor is not None else 'not found'}"
                + (f", pixel-scale keys: {[k for k, _ in header_meta['pixscale_candidates']]}"
                   if header_meta['pixscale_candidates'] else ", no pixel-scale keys"))
    return obs_tensor, src_tensor, header_meta
...
def analyze_single_lens_worker(forward_model_path: str, fits_path: str, output_dir: str, lens_id: str, args):
    ...
    # Instantiate the forward model (v2 architecture -- no init_fwhm/init_beta args;
    # per-observation PSF and lens-light are now encoder-predicted)
    forward_model = ConditionalPhysicalForward(
        ...
        use_per_obs_psf=getattr(args, 'use_per_obs_psf', True),
        use_lens_light=getattr(args, 'use_lens_light', True),
        psf_fwhm_range=(getattr(args, 'psf_fwhm_min', DEFAULT_PSF_FWHM_RANGE[0]),
                        getattr(args, 'psf_fwhm_max', DEFAULT_PSF_FWHM_RANGE[1])),  # [FIX M4]
    )

    try:
        # [FIX M4] strict load with diagnostics (and a meta psf-range check);
        # --allow-partial-load is the explicit opt-in for mismatched keys
        load_checkpoint_file(forward_model, forward_model_path, label='forward',
                             prefer=('forward_state', 'state_dict'),
                             allow_partial=getattr(args, 'allow_partial_load', False))
    ...
    try:
        obs_tensor, src_tensor, header_meta = load_fits_pair(fits_path,
    ...
    return analyze_single_lens(forward_model, obs_tensor, output_dir, lens_id, args=args,
                               src_tensor=src_tensor, header_meta=header_meta)

def analyze_single_lens(forward_model, obs_tensor, output_dir, lens_id="lens_001", args=None, src_tensor=None, header_meta=None):
    """
    Analyze one lens with source plane reconstruction support:
      ...
      - compute cosmology-based masses and sigma_v
    ...
    with torch.no_grad():
        ...
        try:
            # core API [FIX M1]: returns (B,H,W); reuse the already-encoded
            # parameters so the encoder runs once
            kappa_t = forward_model.compute_convergence_map(
                obs_tensor, lens_params=lens_params_torch)
            kappa_map = kappa_t.cpu().numpy()[0]
        except Exception as e:
    ...
    pixscale_method = getattr(args, 'pixscale_method', 'header') if args is not None else 'header'
    pixel_scale_eff = float(getattr(args, 'pixel_scale', 0.05))
    pixel_scale_source = f'assume ({pixel_scale_eff:.5f})'

    if pixscale_method == 'header':
        # [FIX M3] default: trust the pixel scale recorded in the FITS.
        # Priority: PP_PS_N (post-resampling scale written by normalizer.py)
        # > TR_PS (v2.1 simulator truth) > PIXSCALE > PIXSCAL1 > WCS CD/CDELT.
        cands = (header_meta or {}).get('pixscale_candidates', [])
        if cands:
            key, val = cands[0]
            pixel_scale_eff = float(val)
            pixel_scale_source = f'header:{key}'
            logger.info(f"[{lens_id}] Pixel scale from header {key}: "
                        f"{pixel_scale_eff:.5f} arcsec/pix"
                        + (f" (other candidates: {cands[1:]})" if len(cands) > 1 else ""))
        else:
            logger.warning(f"[{lens_id}] pixscale_method='header' but no pixel-scale "
                           f"keywords found (PP_PS_N/TR_PS/PIXSCALE/PIXSCAL1/CD/CDELT); "
                           f"falling back to --pixel-scale {pixel_scale_eff:.5f}. "
                           f"Run normalizer.py v2.1 on the inputs to embed PP_PS_N.")
            pixel_scale_source = f'assume-fallback ({pixel_scale_eff:.5f})'
    elif pixscale_method == 'psf_fwhm':
        ...
        if est is not None:
            pixel_scale_eff = est
            pixel_scale_source = 'psf_fwhm-estimate'
        else:
        ...
        if target_med is None:
            ...
            # [FIX M3] CIRCULARITY WARNING: this sets the pixel scale so that
            # the model's predicted b equals the target Einstein radius. Any
            # downstream 'recovered theta_E' then matches the target BY
            # CONSTRUCTION and cannot be used as evidence the model works.
            # Prefer pixscale-method='header' (the default); use calibrate
            # only for deliberate anchoring and say so in any write-up.
            logger.warning(f"[{lens_id}] pixscale-method='calibrate' is CIRCULAR for "
                           f"Einstein-radius validation: theta_E will equal "
                           f"--target-median-theta ({target_med}) by construction.")
            b_norm = lens_params_dict['b']
            arcsec_per_norm_req = target_med / max(1e-12, b_norm)
            pixel_scale_eff = arcsec_per_norm_req / (W / 2.0)
            pixel_scale_source = f'calibrate (target {target_med})'
    ...
    # -------------------------------------------------------------------------
    # PATCHED: mass_norm — compute SIE and NFW contributions by integrating
    # their respective convergence maps within the Einstein radius mask.
    # This replaces the previous ad hoc formulas:
    #   sis_mass = b * einstein_arcsec          (wrong units / not a convergence)
    #   nfw_mass = kappa_s * rs_arcsec**2       (pixel-scale-squared artifact)
    # Both formulas produced values that were dimensionally inconsistent and
    # not comparable, causing the NFW term to dominate spuriously.
    # -------------------------------------------------------------------------
    try:
        with torch.no_grad():
            xv_grid = torch.linspace(-1, 1, W, device=device).view(1, 1, W).expand(1, H, W)
            yv_grid = torch.linspace(-1, 1, H, device=device).view(1, H, 1).expand(1, H, W)

            # --- SIE convergence (Kormann 1994) ---
            b_t   = lens_params_torch['b'].view(1, 1, 1)
            q_t   = lens_params_torch['q'].view(1, 1, 1)
            phi_t = lens_params_torch['phi'].view(1, 1, 1)
            x0_t  = lens_params_torch['x0'].view(1, 1, 1)
            y0_t  = lens_params_torch['y0'].view(1, 1, 1)
            rc_t  = 0.01 * torch.ones_like(b_t)
            ...
            r_ell = torch.sqrt(
                q_t ** 2 * x_rot ** 2 + y_rot ** 2 + (q_t * rc_t) ** 2 + 1e-12
            )
            kappa_sie_np = (0.5 * b_t * torch.sqrt(q_t) / r_ell).squeeze().cpu().numpy()

            # --- NFW convergence (Wright & Brainerd 2000) ---
            kappa_nfw_np = forward_model.nfw.nfw_convergence(
                xv_grid, yv_grid,
                lens_params_torch['kappa_s'].view(1),
                lens_params_torch['rs'].view(1),
                lens_params_torch['x0'].view(1),
                lens_params_torch['y0'].view(1),
            ).squeeze().cpu().numpy()

        # Build Einstein radius mask in pixel coordinates
        x0_pix = (lens_params_dict['x0'] + 1.0) * W / 2.0
        y0_pix = (lens_params_dict['y0'] + 1.0) * H / 2.0
        r_e_pix = einstein_arcsec / pixel_scale_eff
        Yg, Xg = np.indices((H, W))
        mask_e = np.sqrt((Xg - x0_pix) ** 2 + (Yg - y0_pix) ** 2) <= r_e_pix

        # Mean convergence within Einstein radius for each component.
        # For a perfect model total_mass should be ~1.0 (definition of Einstein radius).
        sis_kbar   = float(np.mean(kappa_sie_np[mask_e]))  if mask_e.any() else 0.0
        nfw_kbar   = float(np.mean(kappa_nfw_np[mask_e]))  if mask_e.any() else 0.0
        total_kbar = float(np.mean(kappa_map[mask_e]))     if mask_e.any() else 0.0

        mass_norm = {
            'sis_mass':                    sis_kbar,
            'nfw_mass':                    nfw_kbar,
            'total_mass':                  total_kbar,
        ...
        logger.info(
            f"[{lens_id}] mass_norm — SIE κ̄={sis_kbar:.4f}, "
            f"NFW κ̄={nfw_kbar:.4f}, total κ̄={total_kbar:.4f} "
            f"(pixels in mask: {int(mask_e.sum())})"
        )

    except Exception as e:
        logger.warning(f"[{lens_id}] Patched mass_norm computation failed ({e}); falling back to zeros.")
        mass_norm = {
            'sis_mass':                    0.0,
            'nfw_mass':                    0.0,
            'total_mass':                  0.0,
    ...
    outdict = {
        ...
        'halo_info': halo_info,
        'pixel_scale_used_arcsec_per_pix': pixel_scale_eff,
        'pixel_scale_source': pixel_scale_source,  # [NEW M6]
        'truth_parameters': (header_meta or {}).get('truth'),  # [NEW M6]
        'residual_metrics': residual_metrics,
        'source_plane_info': {
            'available': src_tensor is not None,
            'shape': list(src_img.shape) if src_tensor is not None else None,
    ...
    def _np_safe(o):
        # [FIX M7] compute_residual_metrics (and other numpy paths) return
        # np.float32/np.int values, which the stdlib json encoder rejects --
        # the v2.0 JSON write crashed whenever residual metrics were present.
    ...
    with open(json_path, 'w') as f:
        json.dump(outdict, f, indent=2, default=_np_safe)
...
def _np_safe(o):
    # [FIX M7] numpy scalars (np.float32/np.int from compute_residual_metrics,
    # np.nanmean, etc.) are rejected by the stdlib json encoder.
    if isinstance(o, np.floating):
        return float(o)
    if isinstance(o, np.integer):
        return int(o)
    ...
    if isinstance(o, np.bool_):
        return bool(o)
    if isinstance(o, np.generic):      # belt-and-suspenders catch-all
        return o.item()
    raise TypeError(f"Object of type {type(o).__name__} is not JSON serializable")
...
def batch_analyze(forward_model_path, fits_files: List[str], output_dir: str, args):
    ...
    if n_procs == 1:
        ...
        # Instantiate forward model with parameters from args (v2)
        forward_model = ConditionalPhysicalForward(
            kernel_size=args.kernel_size,
            enforce_nonneg=True,
            pixel_scale=0.05,  # Fixed for the model
        ...
        try:
            # [FIX M4] strict load with diagnostics. The old fallback re-loaded
            # with strict=False SILENTLY, which can run a wrong-architecture
            # checkpoint whose new heads are random noise. Mismatches are now
            # an error unless --allow-partial-load is passed explicitly.
            load_checkpoint_file(forward_model, forward_model_path, label='forward',
                                 prefer=('forward_state', 'state_dict'),
                                 allow_partial=getattr(args, 'allow_partial_load', False))
            logger.info("Loaded forward model for batch (single-process).")
        except Exception as e:
            logger.error(f"Cannot load forward model: {e}")
            return []
        ...
        for idx, f in enumerate(tqdm(fits_files, desc="Batch analyze")):
            ...
            try:
                obs_tensor, src_tensor, header_meta = load_fits_pair(f,
                                                       source_hdu_names=getattr(args, 'source_hdu_names',
                                                                               ['RECON', 'SRC', 'SOURCE', 'SOURCE_PLANE']))
                res = analyze_single_lens(forward_model, obs_tensor, output_dir, lens_id, args=args,
                                          src_tensor=src_tensor, header_meta=header_meta)
                results.append(res)
            except Exception as e:
    ...
    if results:
        ...
        with open(summary_path, 'w') as f:
            json.dump(
                {'n': len(results), 'stats': stats,
                 'source_plane_fraction': stats['source_available']['fraction']},
                f, indent=2, default=_np_safe,
            )
...
def main():
    ...
    parser.add_argument('--forward-model', type=str, default=r"C:\Users\mythi\AstroRIM\AstroRIM_2.1\model_holder\cond_forward_finetune2.1_best.pt",)
    parser.add_argument('--input', type=str, default=r"C:\Users\mythi\AstroRIM\AstroRIM_2.1\Test_outputs\rxj1131\Recons\q256_RXJ1131_NICH2H_processed_recon.fits")
    parser.add_argument('--output-dir', type=str, default=r'C:\Users\mythi\AstroRIM\AstroRIM_2.1\Test_outputs\rxj1131\Results')
    parser.add_argument('--batch-size', type=int, default=1)
    parser.add_argument('--max-samples', type=int, default=None)
    parser.add_argument('--pixel-scale', type=float, default=0.203, help='Fallback pixel scale (arcsec/pixel)') # Important conversions: If AWFCV is used, when 256 --> 96 then .1333 for NIC2H 256 --> 96 then .203
    parser.add_argument('--pixscale-method', choices=['header', 'assume', 'psf_fwhm', 'calibrate'],
                        default='assume',
                        help="[FIX M3] 'header' (default) reads PP_PS_N/TR_PS/PIXSCALE/WCS "
                             "from the FITS; 'calibrate' (the old default) is circular for "
                             "Einstein-radius validation and now warns.")
    parser.add_argument('--assumed-seeing', type=float, default=.01, help='Assumed seeing FWHM in arcsec for PSF-based pixel scale estimation')
    parser.add_argument('--target-median-theta', type=float, default=1.6)
    parser.add_argument('--device', type=str, default='cuda' if torch.cuda.is_available() else 'cpu', choices=['cuda','cpu'])
    parser.add_argument('--z-lens', type=float, default=0.295)
    parser.add_argument('--z-source', type=float, default=0.658)
    ...
    # [FIX M5] these were store_true with default=True and could never be
    # disabled from the CLI; now --x / --no-x
    parser.add_argument('--use-nfw', action=argparse.BooleanOptionalAction, default=True, help='Enable NFW halo in forward operator')
    parser.add_argument('--use-subhalos', action=argparse.BooleanOptionalAction, default=False, help='Enable subhalos in forward operator')
    parser.add_argument('--n-subhalos', type=int, default=2, help='Number of subhalos')
    parser.add_argument('--use-per-obs-psf', action=argparse.BooleanOptionalAction, default=True,
                        help='Use encoder-predicted per-observation PSF (must match training config)')
    parser.add_argument('--use-lens-light', action=argparse.BooleanOptionalAction, default=True,
                        help='Include Sersic lens-light term in forward operator (must match training config)')
    parser.add_argument('--kernel-size', type=int, default=21, help='PSF kernel size')
    parser.add_argument('--learn-residual-psf', action=argparse.BooleanOptionalAction, default=True, help='Learn residual PSF corrections')
    # [FIX M4]
    parser.add_argument('--psf-fwhm-min', type=float, default=DEFAULT_PSF_FWHM_RANGE[0],
                        help='Encoder PSF FWHM lower bound in detector px; pass 1.0 for legacy v2.0 checkpoints')
    parser.add_argument('--psf-fwhm-max', type=float, default=DEFAULT_PSF_FWHM_RANGE[1])
    parser.add_argument('--allow-partial-load', action='store_true', default=False,
                        help='Explicit opt-in to load a checkpoint with missing/unexpected keys '
                             '(the loader prints exactly which tensors stay at initialization)')
